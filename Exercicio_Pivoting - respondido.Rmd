---
title: "Exercicio Pivoting - com respostas"
...
- Na tabela table1, cada linha representa uma combinação (`country`, `year`). As colunas `cases` e `population` contêm os valores dessas variáveis.

- Na tabela 2, cada linha representa uma combinação (`country`, `year`, `variable`). A `count` de colunas contém os valores dos casos de variáveis e população em linhas separadas.

- Na tabela 3, cada linha representa uma combinação (`country`, `year`). A coluna `rate` fornece os valores de casos e população em uma string formatada como casos / população.

2. Calcule a taxa para table2 e table4a + table4b. Você precisará realizar quatro operações:

2.1 Extraia o número de casos de TB por país por ano.

Para calcular os casos por pessoa, precisamos dividir os casos pela população para cada país e ano. Isso é mais fácil se os casos e as variáveis de população forem duas colunas em um quadro de dados em que as linhas representam combinações (país, ano).

*Passo 1 - crie tabelas em separado para "cases" e "population" e garanta que elas estão organizadas na mesma ordem, via "arrange".*
```{r}
...
2.2 Extraia a população correspondente por país por ano.
2.3 Divida os casos pela população e multiplique por 10.000.

*Passo 2 - agora crie um novo dataframe com colunas - população e casos, e calcule o numero de casos per capita em uma nova coluna*.
...
2.4 Guarde em local apropriado. # errado
*Passo 3 - Salve a nova variavel no local apropriado - também adicionaremos novas linhas a tabela `table2`*.
...
2.5 Qual representação é mais fácil de trabalhar? Qual é o mais difícil? Por quê?

Primeiro vamos ver cada tabela.
...
*Para table4a e table4b, crie uma nova tabela para casos per capita, que chamaremos de table4c, com linhas de país e colunas de ano*.
...
Conclusão: Nenhuma das tabelas é particularmente fácil de trabalhar. Como a tabela2 tem linhas separadas para casos e população, precisamos gerar uma tabela com colunas para casos e população onde poderíamos calcular os casos per capita. a tabela4a e a tabela4b dividem os casos e as variáveis de população em diferentes tabelas, o que torna fácil dividir os casos por população. No entanto, tivemos que repetir esse cálculo para cada linha.

O formato ideal de um quadro de dados para responder a essa pergunta é aquele com as colunas país, ano, casos e população. Então, o problema poderia ser respondido com uma única chamada `mutate()`.


3. Se você quiser se desafiar um pouco mais, tente recriar o gráfico mostrando a mudança nos casos ao longo do tempo usando a `table2` em vez da `table1`. O que você precisa fazer primeiro?
...
```{r}
# Resposta
dataset <- as_tibble(list(
  nome = c("Lorena Almeida","Tania Almeida", "Cleise Almeida", "Clesia Almeida"), 
  dados = c("idade", "altura", "idade", "altura"), 
  valores = c(45, 186, 37, 156) ))
```
...
```{r}
# Devemos usar a função `pivot_wider`
  dataset %>% 
    pivot_wider(names_from = dados, values_from = valores)
```

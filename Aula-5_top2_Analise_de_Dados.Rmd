...
```{r}
dados2 <- dados %>% 
  filter(Country.Region %in% c("Brazil", "Argentina", "Chile", "US")) %>% # podemos retirar esse filtro.
...
```{r}
# dados2 %>% 
#   mutate(dia = str_replace(dia, "X", "")) %>%
#   lubridate::as_date(dados2$dia, format = "%m.%d.%y")
...
dados3 <- dados2 %>% 
  ...
  filter(n_casos > 0)  %>%
  group_by(Country.Region) %>% # Agrupando para cada país
  mutate(dia0 = min(dia))  # VDefinindo o dia 0 como o menor dia do país em questão 
```
...
```{r}
dados7 %>% 
  # filter(Country.Region %in% c("Argentina"))  %>% 
  group_by(conta_dia) %>%
...
p <- dados7 %>% 
  filter(Country.Region %in% c("US", "Brazil", "Chile")) %>% 
  group_by(conta_dia)
